...
def update_sensor_name():
    ...
    with sqlite3.connect('vitrina.db') as conn:
        ...
        c.execute('''INSERT INTO sensor_names_history 
                    (sensor_id, old_name, new_name, change_date) 
                    SELECT sensor_id, product_name, ?, datetime('now')
                    FROM sensor_names 
                    WHERE sensor_id = ?''', (name, sensor_id))
                    
        # Actualizar nombre actual
        c.execute('''INSERT OR REPLACE INTO sensor_names 
                    (sensor_id, product_name) VALUES (?, ?)''', 
                    (sensor_id, name))
        conn.commit()
    return jsonify({"success": True})
...
def init_db():
    with sqlite3.connect('vitrina.db') as conn:
        c = conn.cursor()
        # Agregar tabla de historial de nombres
        c.execute('''CREATE TABLE IF NOT EXISTS sensor_names_history
                    (id INTEGER PRIMARY KEY AUTOINCREMENT,
                     sensor_id INTEGER,
                     old_name TEXT,
                     new_name TEXT,
