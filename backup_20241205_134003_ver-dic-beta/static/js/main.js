...
async function handleAutoplay() {
    try {
        ...
        debugLog('Esperando interacción del usuario para reproducir videos');
        showPlayPrompt();
...
async function tryPlayVideo(video) {
    ...
    try {
        // Siempre empezar muted
        video.muted = true;
        await video.play();
        
        // Intentar desmutear solo si es necesario
        const sensorId = video.id.replace(/\D/g, '');
        if (localStorage.getItem(`sensor_${sensorId}_muted`) !== 'true') {
            video.muted = false;
        }
    } catch (error) {
...
async function switchToSingleMode(sensorId) {
    ...
    try {
        const response = await fetch(`/api/sensor_status/${sensorId}`);
        const data = await response.json();

        if (!data.video_path) throw new Error('No hay video asignado');
...
async function switchToTripleMode(sensors) {
    ...
    try {
        ...
        const position = extraContent.position || 'bottom-right';
