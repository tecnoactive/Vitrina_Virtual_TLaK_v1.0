    ...
    fetch('/api/extra-content')
        ...
        .then(data => {
            infoDiv.innerHTML = `
                ...
                <p>Posición: ${data.position || 'No definida'}</p>
...
async function updateVersusMode() {
    ...
    try {
        ...
        if (response.ok) {
            // Mostrar sección extra para modos 3 y 4
            extraSection.style.display = ['3', '4'].includes(mode) ? 'block' : 'none';
            showSuccess(`Modo actualizado: ${modeNames[mode]}`);
        } else {
            throw new Error('Error al actualizar');
        }
    } catch (error) {
        showError('Error al actualizar modo versus');
    } finally {
        hideLoading();
    }
}
...
async function loadCurrentMode() {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('versus-mode').value = data.mode.toString();
            document.getElementById('extra-content-section').style.display = 
                data.mode === 3 ? 'block' : 'none';
        }
    } catch (error) {
